...
The full design system bundle lives at `docs/design/`. All paths in this skill are relative to the repo root.

## Always do first

1. Read `docs/design/README.md` — full context, content fundamentals, and visual foundations.
2. Link `docs/design/colors_and_type.css` from any new HTML file. It pulls Schibsted Grotesk + JetBrains Mono from Google Fonts and exposes every token.
3. Skim the relevant `docs/design/ui_kits/<surface>/` for prebuilt chrome before building from scratch. The three kits implement the same tokens differently — match the right one.
4. If a flow you're implementing matches one in `docs/design/prototypes/`, read that prototype top-to-bottom — it shows the intended interaction, copy, and webhook side-effects, not just the visual.

## Non-negotiable vocabulary

Use the canonical domain words verbatim — `Integrator`, `Customer`, `Product Type`, `Dimension`, `Plan` / `Plan Version`, `Subscription`, `Event`, `Rollup`, `Threshold`, `Invoice`, `Line Item`, `Period Close`, `Final Close`. Never use `tenant`, `plan revision`, `environment`, `sub`, `metric`, or `record`. The full Avoid list is in `CONTEXT.md`.
...
- **Icons**: use Lucide subset rendered inline as 1.5px stroke / `currentColor` SVG. Manifest in `docs/design/assets/icons/icon-manifest.json`. Never invent custom icons; if a metaphor is missing, ask.
...
- **Admin Dashboard** (`docs/design/ui_kits/admin/`): full chrome, sidebar nav, dense data tables, sandbox status bar when applicable. Brand pink on primary actions.
- **Customer Portal** (`docs/design/ui_kits/portal/`): single-column, max 1080px, no sidebar, embed-friendly. Co-branded with the Integrator's mark in the page head plus a small "powered by Saasy" foot. End customers do not authenticate to Saasy directly.
- **Ops Console** (`docs/design/ui_kits/ops/`): dark, mono-first, terminal-adjacent. Pink reserved for "act now" affordances (firing thresholds, retry buttons). Per ADR-0014 not held to customer-facing polish standards.
